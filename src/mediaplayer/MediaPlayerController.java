...
public class MediaPlayerController implements Initializable {
    ...
    Map<Integer, String> grandFileMap = new HashMap<Integer, String>();

    int currentFileNumber;

    int fileMapMaxKey;

    int grandFileMapMaxKey;
        ...
        if (file.exists()) {
            ...
            if (parentFile.isDirectory()) {
                ...
                for (File file : listFiles) {
                    ...
                    if (fileMapMaxKey < fileNumber) {
                        fileMapMaxKey = fileNumber;
                    }
                }
            }
    ...
    public void play(String mediaFile) {
        ...
        mediaPlayer.setOnEndOfMedia(new Runnable() {
            public void run() {
                ...
                boolean foundNextFile = false;
                while (!(currentFileNumber == fileMapMaxKey) && !foundNextFile) {
                    if (fileMap.get(currentFileNumber + 1) != null) {
                        foundNextFile = true;
                        break;
                    } else {
                        currentFileNumber++;
                ...
                if (fileMap.get(currentFileNumber + 1) != null) {
                    ...
                    String nextFilePath = fileMap.get(currentFileNumber);

                    // playLoader(mediaPlayer);

                    File newFile = new File(nextFilePath);
                    play(newFile.toURI().toString());
                } else {
                    System.out.println("Searching in new folder....." + currentFileNumber);
                    String currentFilePath = fileMap.get(currentFileNumber);
                    System.out.println("Searching in new folder....." + currentFilePath);
                    ...
                    if (grandParentFile.exists()) {
                        ...
                        if (grandParentFile.isDirectory()) {
                            File[] listFiles = grandParentFile.listFiles();
                            grandFileMap = new HashMap<Integer, String>();
                            for (File file : listFiles) {
                                if (file.isDirectory()) {
                                    int fileNumber = getFileNumber(file.getName());
                                    grandFileMap.put(fileNumber, file.getAbsolutePath());
                                }
                            }

                            int currentParentNumber = getFileNumber(parentFile.getName());
                            String nextParentPath = grandFileMap.get(currentParentNumber + 1);

                            if (nextParentPath != null) {
                                File newFile = new File(nextParentPath);
                                File[] adjacentFolderFiles = newFile.listFiles(fileNameFilter);
                                fileMap = new HashMap<Integer, String>();
                                for (File file : adjacentFolderFiles) {
                                    int fileNumber = getFileNumber(file.getName());
                                    fileMap.put(fileNumber, file.getAbsolutePath());
                                }

                                String newMediaFile = fileMap.get(currentFileNumber + 1);
                                if (newMediaFile != null) {

                                    // playLoader(mediaPlayer);

                                    File newFileToPlay = new File(newMediaFile);
                                    play(newFileToPlay.toURI().toString());
                                } else {
                                    return;
                                }
                            }
    ...
    private void playLoader(MediaPlayer mediaPlayer) {
        System.out.println("Inside Loading....");
        File newFile = new File("C:/work/Projects/Consum/eclipsews/MediaPlayer/src/videos/laoder.mp4");
        Media media = new Media(newFile.toURI().toString());
        mediaPlayer = new MediaPlayer(media);
        String source = mediaPlayer.getMedia().getSource();
